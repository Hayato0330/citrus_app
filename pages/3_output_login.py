...
for idx, row in enumerate(top_items.itertuples(), start=1):
    with quadrants[idx - 1]:
        st.markdown(f"#### {idx}. {row.Item_name}")
        ...
        # 外部リンク
        st.markdown(f"[Amazonで見る]({build_amazon_url(row.Item_name)})", unsafe_allow_html=True)
        st.markdown(f"[楽天で見る]({build_rakuten_url(row.Item_name)})", unsafe_allow_html=True)
        st.markdown(f"[さとふるで見る]({build_satofuru_url(row.Item_name)})", unsafe_allow_html=True)


# === 右下「まとめ」ブロック ===
with quadrants[3]:
    st.markdown("#### まとめ")
    citrus_names = [r.Item_name for r in top_items.itertuples()]
